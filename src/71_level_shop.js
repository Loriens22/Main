...
(function (SG, THREE) {
  ...
  function buildShop(ctx, opts) {
    ...
    K.room(ctx, {
      ...
      floorTag: 'carpet',
      openings: {
        /* South wall: the shop front — a door and two big panes. */
        s: [
          { at: 3.4, w: 1.05, y0: 0, y1: 2.1 },                 /* door */
          { at: 6.0, w: 3.6, y0: 0.55, y1: 2.35, noReveal: true }, /* window L */
          { at: 1.2, w: 1.8, y0: 0.55, y1: 2.35, noReveal: true }  /* window R */
        ],
        n: [{ at: 1.0, w: 0.95, y0: 0, y1: 2.05 }]              /* storeroom */
      }
    });
    ...
    K.box(glass, glassMat, -2.9, 1.45, Z0 + 0.02, 3.6, 1.8, 0.012);
    K.box(glass, glassMat, 2.2, 1.45, Z0 + 0.02, 1.8, 1.8, 0.012);
    ...
    [-4.72, -2.9, -1.08, 1.28, 3.12].forEach(function (x) {
      K.box(mull, alu, x, 1.45, Z0 + 0.02, 0.06, 1.86, 0.06);
    });
    K.box(mull, alu, -1.4, 0.53, Z0 + 0.02, 8.2, 0.07, 0.07);
    K.box(mull, alu, -1.4, 2.38, Z0 + 0.02, 8.2, 0.07, 0.07);
    K.freeze(ctx, mull);
    ...
    chimeBox.position.set(-2.1, 2.2, Z0 + 0.1);
    ctx.add(chimeBox);
  ...
  function buildLot(ctx) {
    ...
    /* Kerb + a strip of grass so the lot has an edge. */
    K.box(lot, K.M('concrete', { color: 0x8d8a84, roughness: 0.95 }),
      -1, 0.06, -5.4, 22, 0.12, 0.5);
    ctx.box(-1, 0.06, -5.4, 22, 0.14, 0.5, 0, 'concrete');

    /* The shop's own facade, seen from outside through the glass. */
    var facade = new THREE.Group();
    K.box(facade, K.M('brickPainted', { color: 0xa8a094, roughness: 0.95 }),
      0, 1.9, -4.45, 13.5, 3.8, 0.4);
