...
export function buildTramRoute() {
  ...
  const pj1 = J(sJ1), pj2 = J(sJ2), pj3 = J(sJ3);
  const perp = (p, len, s0) => new Turtle(p.x - Math.cos(p.h + Math.PI / 2) * s0, p.z - Math.sin(p.h + Math.PI / 2) * s0, p.h + Math.PI / 2).line(len).build();
  const X1 = mk('X1', 'бул. „Гоце Делчев“', perp(pj1, 840, 420), { halfW: 7 });
  const X2 = mk('X2', 'ул. „Хубча“', perp(pj2, 520, 260), { halfW: 7 });
  ...
  const s0 = stopAt(B1, 58);
  const rs = (raw) => raw - s0;
  const stops = [
    { id: 'ml', name: 'кв. Манастирски ливади', short: 'М. ЛИВАДИ', tts: 'квартал Манастирски ливади', link: 'връзка с трамвай № 27', linkTts: 'връзка с трамвай номер двадесет и седем', s: 0, st: B1, sb: 58 },
  ...
  // tram stop lines before the three junctions
  const stopLines = [
    { inter: inters[0], group: 'A', s: rs(stopAt(B1, sJ1 - 16)), turn: null },
    { inter: inters[1], group: 'A', s: rs(stopAt(B1, sJ2 - 16)), turn: null },
    { inter: inters[2], group: 'A', s: rs(stopAt(B1, sJ3 - 24)), turn: 'left', yield: false },
  ];
  ...
  lanesFor(B1, 'B1', [RES + 1.75, RES + 5.25, RES + 8.75]);
  lanesFor(B2, 'B2', [RES + 1.75, RES + 5.25, RES + 8.75]);
  lanesFor(X1, 'X1', [1.75, 5.25]);
  lanesFor(X2, 'X2', [1.75, 5.25]);
  // signals per lane: stop before the crossing street (its full width incl. sidewalks + zebra)
  for (const L of lanes) {
    for (const it of inters) {
      const arm = [it.a, it.b].find((a) => a.st === L.street); if (!arm) continue;
      const other = arm === it.a ? it.b : it.a;
      const dist = other.st.halfW + other.st.sidewalk + 3.5;
      const p = arm.st.poly.at(arm.s - L.dir * dist);
  ...
  const route = {
    ...
    bus: track, busOffset: s0, opp, S: { B1, B2, X1, X2 }, J: { sJ1, sJ2, sJ3 }, loopEnd: loop,
