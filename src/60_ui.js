...
(function () {
  ...
  var DEFAULT_SETTINGS = {
    ...
    autoSprint: true, touchScale: 1.0, touchOpacity: 0.85,
    lookMode: 'stick', inputDebug: false,
    /* accessibility */
  ...
  function hudCSS() {
    ...
    return [
      ...
      /* ---- HUD ---- */
      '#ip-ui .ip-hud{position:absolute;inset:0;pointer-events:none;opacity:1;transition:opacity .25s;}',
      '#ip-ui .ip-hud.off{opacity:0;}',
      '#ip-ui .ip-vit{position:absolute;left:calc(18px + env(safe-area-inset-left, 0px));bottom:calc(18px + env(safe-area-inset-bottom, 0px));width:190px;}',
      '#ip-ui .ip-segs{display:flex;gap:3px;height:9px;}',
      '#ip-ui .ip-segs i{flex:1;background:#1b1f26;border:1px solid #2b3038;position:relative;overflow:hidden;}',
      '#ip-ui .ip-segs i b{position:absolute;inset:0;transform-origin:left;background:' + P.hp + ';transition:transform .12s linear;}',
      '#ip-ui .ip-segs i u{position:absolute;inset:0;transform-origin:left;background:' + P.hpGhost + ';opacity:.55;transition:transform .5s ease .25s;}',
      '#ip-ui .ip-stam{margin-top:5px;height:3px;background:#161a20;}',
      '#ip-ui .ip-stam b{display:block;height:100%;background:' + P.stam + ';transform-origin:left;}',
      '#ip-ui .ip-ammo{position:absolute;right:calc(18px + env(safe-area-inset-right, 0px));bottom:calc(18px + env(safe-area-inset-bottom, 0px));text-align:right;}',
      '#ip-ui .ip-ammo .mag{font-size:34px;font-weight:700;line-height:1;color:' + P.ammo + ';letter-spacing:.04em;}',
      '#ip-ui .ip-ammo .res{font-size:13px;opacity:.62;letter-spacing:.18em;}',
      '#ip-ui .ip-ammo .wep{font-size:11px;opacity:.5;letter-spacing:.26em;text-transform:uppercase;margin-bottom:4px;}',
      '#ip-ui .ip-ammo.low .mag{color:' + P.warn + ';}',
      /* companion */
      '#ip-ui .ip-comp{position:absolute;left:calc(18px + env(safe-area-inset-left, 0px));bottom:calc(62px + env(safe-area-inset-bottom, 0px));display:flex;align-items:center;gap:8px;}',
      '#ip-ui .ip-ring{width:34px;height:34px;border-radius:50%;border:2px solid ' + P.ok + ';position:relative;flex:0 0 auto;background:#0c1015;}',
      '#ip-ui .ip-ring b{position:absolute;left:2px;right:2px;bottom:2px;background:' + P.ok + ';opacity:.30;border-radius:0 0 16px 16px;}',
      '#ip-ui .ip-ring.warn{border-color:' + P.warn + ';} #ip-ui .ip-ring.warn b{background:' + P.warn + ';}',
      '#ip-ui .ip-ring.bad{border-color:' + P.bad + ';animation:ipPulse .7s infinite;} #ip-ui .ip-ring.bad b{background:' + P.bad + ';}',
      '@keyframes ipPulse{0%,100%{opacity:1}50%{opacity:.42}}',
      '#ip-ui .ip-ring i{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:12px;font-style:normal;letter-spacing:.06em;}',
      '#ip-ui .ip-compname{font-size:10px;letter-spacing:.22em;opacity:.6;text-transform:uppercase;}',
      '#ip-ui .ip-arrow{position:absolute;width:0;height:0;border-left:7px solid transparent;border-right:7px solid transparent;border-bottom:12px solid ' + P.warn + ';opacity:0;transition:opacity .2s;}',
      /* objective + clock */
      '#ip-ui .ip-obj{position:absolute;top:calc(16px + env(safe-area-inset-top, 0px));left:50%;transform:translateX(-50%);text-align:center;max-width:74vw;}',
      '#ip-ui .ip-obj .lbl{font-size:9px;letter-spacing:.42em;opacity:.42;text-transform:uppercase;}',
      '#ip-ui .ip-obj .txt{font-size:13px;letter-spacing:.06em;opacity:.86;margin-top:3px;}',
      '#ip-ui .ip-clock{position:absolute;top:calc(16px + env(safe-area-inset-top, 0px));right:calc(18px + env(safe-area-inset-right, 0px));font-size:20px;letter-spacing:.10em;color:' + P.bad + ';display:none;}',
      /* reticle */
      '#ip-ui .ip-ret{position:absolute;left:50%;top:50%;width:64px;height:64px;margin:-32px 0 0 -32px;opacity:0;transition:opacity .12s;}',
      '#ip-ui .ip-ret.on{opacity:1;}',
      '#ip-ui .ip-ret span{position:absolute;background:' + P.ok + ';box-shadow:0 0 4px rgba(0,0,0,.9);}',
      '#ip-ui .ip-ret.hostile span{background:' + P.bad + ';}',
      '#ip-ui .ip-dot{position:absolute;left:50%;top:50%;width:4px;height:4px;margin:-2px 0 0 -2px;border-radius:50%;background:#ff2b1d;box-shadow:0 0 8px #ff2b1d,0 0 20px rgba(255,43,29,.55);opacity:0;}',
      '#ip-ui .ip-dot.on{opacity:1;}',
      /* prompts, toasts, subtitles */
      '#ip-ui .ip-prompt{position:absolute;left:50%;top:58%;transform:translateX(-50%);font-size:13px;letter-spacing:.10em;background:rgba(5,7,10,.72);padding:7px 14px;border:1px solid #2a3038;border-radius:3px;display:none;}',
      '#ip-ui .ip-prompt kbd{display:inline-block;min-width:19px;padding:1px 5px;margin-right:8px;border:1px solid #4a525c;border-radius:3px;background:#161a20;font:inherit;font-size:11px;}',
      '#ip-ui .ip-toasts{position:absolute;top:calc(74px + env(safe-area-inset-top, 0px));left:50%;transform:translateX(-50%);display:flex;flex-direction:column;gap:5px;align-items:center;}',
      '#ip-ui .ip-toast2{font-size:11px;letter-spacing:.20em;text-transform:uppercase;background:rgba(5,7,10,.80);border-left:2px solid ' + P.warn + ';padding:6px 13px;opacity:0;transform:translateY(-6px);transition:opacity .3s,transform .3s;}',
      '#ip-ui .ip-toast2.on{opacity:1;transform:none;}',
      '#ip-ui .ip-subs{position:absolute;left:50%;bottom:calc(96px + env(safe-area-inset-bottom, 0px));transform:translateX(-50%);width:min(760px,86vw);text-align:center;}',
      '#ip-ui .ip-subline{display:inline-block;padding:5px 12px;border-radius:3px;line-height:1.45;}',
      '#ip-ui .ip-subline .sp{font-weight:700;letter-spacing:.16em;margin-right:9px;}',
      /* damage + horror overlays */
      '#ip-ui .ip-vig{position:absolute;inset:0;pointer-events:none;background:radial-gradient(ellipse at center,rgba(0,0,0,0) 42%,rgba(120,0,0,.62) 100%);opacity:0;transition:opacity .28s;}',
      '#ip-ui .ip-dmgdir{position:absolute;left:50%;top:50%;width:200px;height:200px;margin:-100px 0 0 -100px;opacity:0;}',
      '#ip-ui .ip-dmgdir b{position:absolute;left:50%;top:0;width:52px;height:16px;margin-left:-26px;background:linear-gradient(to bottom,rgba(220,40,30,.95),rgba(220,40,30,0));}',
      /* ---- TOUCH CONTROLS ---- */
      '#ip-ui .ip-touch{position:absolute;inset:0;display:none;}',
      '#ip-ui .ip-touch.on{display:block;}',
      '#ip-ui .ip-stick{position:absolute;width:132px;height:132px;border-radius:50%;border:2px solid rgba(215,211,200,.22);background:rgba(10,13,18,.28);opacity:0;transition:opacity .15s;}',
      '#ip-ui .ip-stick.on{opacity:1;}',
      '#ip-ui .ip-stick i{position:absolute;left:50%;top:50%;width:54px;height:54px;margin:-27px 0 0 -27px;border-radius:50%;background:rgba(215,211,200,.30);border:1px solid rgba(215,211,200,.45);}',
      '#ip-ui .ip-tbtn{position:absolute;border-radius:50%;background:rgba(12,16,21,.52);border:1.5px solid rgba(215,211,200,.30);color:#d7d3c8;font-size:10px;letter-spacing:.10em;text-transform:uppercase;display:flex;align-items:center;justify-content:center;text-align:center;pointer-events:auto;user-select:none;line-height:1.15;}',
      '#ip-ui .ip-tbtn.hit{background:rgba(184,35,43,.55);border-color:#e8564d;transform:scale(.94);}',
      '#ip-ui .ip-tbtn.big{width:82px;height:82px;font-size:12px;}',
      '#ip-ui .ip-tbtn.mid{width:62px;height:62px;}',
      '#ip-ui .ip-tbtn.sm{width:52px;height:52px;font-size:9px;}',
      '#ip-ui .ip-tbtn.ctx{background:rgba(20,40,30,.62);border-color:rgba(111,227,154,.55);}',
      '#ip-ui .ip-radial{position:absolute;width:224px;height:224px;margin:-112px 0 0 -112px;display:none;}',
      '#ip-ui .ip-radial.on{display:block;}',
      '#ip-ui .ip-radial b{position:absolute;width:64px;height:64px;margin:-32px 0 0 -32px;border-radius:50%;background:rgba(12,16,21,.86);border:1.5px solid rgba(215,211,200,.34);display:flex;align-items:center;justify-content:center;font-size:9px;letter-spacing:.08em;text-transform:uppercase;font-weight:400;}',
      '#ip-ui .ip-radial b.sel{background:rgba(184,35,43,.62);border-color:#e8564d;}',
      '#ip-ui .ip-qte{position:absolute;left:50%;top:50%;width:180px;height:180px;margin:-90px 0 0 -90px;border-radius:50%;border:3px solid ' + P.bad + ';display:none;align-items:center;justify-content:center;font-size:16px;letter-spacing:.20em;text-transform:uppercase;background:rgba(120,10,10,.30);pointer-events:auto;}',
      '#ip-ui .ip-qte.on{display:flex;animation:ipPulse .35s infinite;}',
      '#ip-ui .ip-dbg{position:absolute;left:50%;top:44%;transform:translateX(-50%);',
      '  font-size:10px;line-height:1.5;letter-spacing:.06em;color:#7fdca0;',
      '  background:rgba(4,6,9,.86);border:1px solid #23303a;padding:6px 10px;',
      '  white-space:pre;display:none;z-index:70;font-variant-numeric:tabular-nums;}',
      '#ip-ui .ip-dbg.on{display:block;}',
      /* ---- screens ---- */
      /* Landscape phones give ~390px of HEIGHT. Every screen is a column with
         a compact header, one scrolling body, and a footer that can never be
         overlapped by the body. */
      '#ip-ui .ip-scr{position:absolute;inset:0;display:none;flex-direction:column;align-items:stretch;justify-content:flex-start;background:rgba(4,6,9,.92);pointer-events:auto;overflow:hidden;',
      '  padding:calc(16px + env(safe-area-inset-top, 0px)) calc(18px + env(safe-area-inset-right, 0px))',
      '          calc(10px + env(safe-area-inset-bottom, 0px)) calc(18px + env(safe-area-inset-left, 0px));}',
      '#ip-ui .ip-scr.on{display:flex;}',
      '#ip-ui .ip-scr>*{flex:0 0 auto;}',
      '#ip-ui .ip-scr h1{font-size:clamp(15px,4.4vh,30px);letter-spacing:.20em;text-transform:uppercase;',
      '  margin:0 0 2px;text-align:center;line-height:1.1;}',
      '#ip-ui .ip-scr h3{font-size:clamp(8px,1.7vh,10px);letter-spacing:.44em;text-transform:uppercase;',
      '  color:' + P.bad + ';margin:0 0 8px;text-align:center;}',
      /* body scrolls, footer does not */
      '#ip-ui .ip-body{flex:1 1 auto;min-height:0;overflow-y:auto;-webkit-overflow-scrolling:touch;',
      '  display:flex;flex-direction:column;align-items:center;gap:4px;width:100%;}',
      /* opaque so a scrolling body cannot show through it */
      '#ip-ui .ip-foot{flex:0 0 auto;display:flex;flex-wrap:wrap;gap:8px;justify-content:center;',
      '  padding:10px 0 2px;width:100%;background:rgba(4,6,9,.97);',
      '  box-shadow:0 -12px 18px rgba(4,6,9,.97);position:relative;z-index:2;}',
      '#ip-ui .ip-mbtn{display:flex;align-items:center;flex:0 0 auto;min-height:44px;',
      '  width:min(300px,74vw);margin:0;padding:10px 16px;background:rgba(16,20,26,.92);',
      '  border:1px solid #2b323b;border-left:3px solid ' + P.bad + ';color:#d7d3c8;font:inherit;',
      '  font-size:11px;letter-spacing:.20em;text-transform:uppercase;text-align:left;',
      '  cursor:pointer;pointer-events:auto;}',
      '#ip-ui .ip-foot .ip-mbtn{width:auto;min-width:140px;justify-content:center;text-align:center;}',
      '#ip-ui .ip-mbtn:hover,#ip-ui .ip-mbtn:focus{background:rgba(30,36,45,.95);outline:none;}',
      '#ip-ui .ip-mbtn[disabled]{opacity:.35;cursor:default;}',
      '#ip-ui .ip-opt{display:flex;align-items:center;justify-content:space-between;gap:12px;',
      '  width:min(560px,94vw);min-height:44px;padding:4px 6px;',
      '  border-bottom:1px solid rgba(255,255,255,.07);font-size:11px;letter-spacing:.12em;',
      '  text-transform:uppercase;pointer-events:auto;}',
      '#ip-ui .ip-opt>span:first-child{flex:1 1 auto;opacity:.86;}',
      '#ip-ui .ip-opt .val{flex:0 0 auto;min-width:56px;text-align:right;opacity:.62;',
      '  font-variant-numeric:tabular-nums;}',
      /* custom range: the default browser blue is completely off-style */
      '#ip-ui .ip-opt input[type=range]{flex:0 0 auto;width:min(190px,38vw);height:34px;',
      '  -webkit-appearance:none;appearance:none;background:transparent;pointer-events:auto;margin:0;}',
      '#ip-ui .ip-opt input[type=range]::-webkit-slider-runnable-track{height:3px;background:#232a33;}',
      '#ip-ui .ip-opt input[type=range]::-moz-range-track{height:3px;background:#232a33;}',
      '#ip-ui .ip-opt input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;',
      '  width:16px;height:22px;margin-top:-10px;background:' + P.bad + ';border:1px solid #e8564d;border-radius:1px;}',
      '#ip-ui .ip-opt input[type=range]::-moz-range-thumb{width:16px;height:22px;background:' + P.bad + ';',
      '  border:1px solid #e8564d;border-radius:1px;}',
      '#ip-ui .ip-optbtn{flex:0 0 auto;min-width:104px;min-height:36px;padding:8px 14px;',
      '  background:#161a20;border:1px solid #3a434e;border-left:2px solid ' + P.bad + ';',
      '  color:#d7d3c8;font:inherit;font-size:10px;letter-spacing:.14em;cursor:pointer;',
      '  pointer-events:auto;text-transform:uppercase;}',
      '#ip-ui .ip-optbtn:active{background:#242c36;}',
      '#ip-ui .ip-sect{width:min(560px,94vw);margin:10px 0 2px;font-size:9px;letter-spacing:.40em;',
      '  text-transform:uppercase;color:' + P.bad + ';opacity:.8;border-bottom:1px solid rgba(184,35,43,.28);',
      '  padding-bottom:4px;}',
      /* auto margins centre it vertically when it fits, and still scroll when it does not */
      '#ip-ui .ip-invrow{display:flex;gap:14px;align-items:flex-start;justify-content:center;',
      '  width:100%;flex-wrap:wrap;margin:auto 0;}',
      '#ip-ui .ip-side2{flex:1 1 200px;max-width:300px;min-width:180px;text-align:left;',
      '  border-left:1px solid rgba(255,255,255,.08);padding-left:12px;}',
      '#ip-ui .ip-side2 .nm{font-size:12px;letter-spacing:.12em;text-transform:uppercase;margin-bottom:6px;}',
      '#ip-ui .ip-side2 .ds{font-size:10px;line-height:1.7;opacity:.66;letter-spacing:.04em;}',
      '#ip-ui .ip-grid{display:grid;gap:2px;background:#0b0e13;border:1px solid #2b323b;padding:4px;flex:0 0 auto;}',
      '#ip-ui .ip-gcell{background:rgba(255,255,255,.028);border:1px solid rgba(255,255,255,.05);}',
      '#ip-ui .ip-gitem{position:absolute;background:rgba(40,48,58,.95);border:1px solid #59636f;font-size:8px;letter-spacing:.04em;display:flex;align-items:center;justify-content:center;text-align:center;padding:2px;cursor:grab;pointer-events:auto;overflow:hidden;}',
      '#ip-ui .ip-gitem.sel{border-color:' + P.warn + ';background:rgba(70,58,32,.95);}'
    ].join('\n');
  }
  ...
  var IN = {
    ...
    touchLook: { id: -1, lx: 0, ly: 0, dx: 0, dy: 0, idle: 0,
                 ox: 0, oy: 0, sx: 0, sy: 0 },
  ...
  /* ---- touch --------------------------------------------------------- */
  var touchLayer = null, stickEl = null, lookStickEl = null, radialEl = null, qteEl = null;
  var RADIAL_CMDS = [
  ...
  function initTouch() {
    ...
    lookStickEl = mk('div', 'ip-stick', touchLayer, null);
    mk('i', null, lookStickEl, null);
    ...
    sty(invB, side, '26px'); sty(invB, 'top', 'calc(84px + env(safe-area-inset-top, 0px))');
    ...
    sty(pauseB, far, '18px'); sty(pauseB, 'top', 'calc(14px + env(safe-area-inset-top, 0px))');
    ...
    sty(compB, far, '18px'); sty(compB, 'bottom', 'calc(148px + env(safe-area-inset-bottom, 0px))');
    ...
    function down(e) {
      ...
      for (var i = 0; i < touches.length; i++) {
        ...
        if (leftSide && IN.stick.id < 0) {
          ...
          IN.touchLook.ox = t.clientX; IN.touchLook.oy = t.clientY;
          IN.touchLook.sx = 0; IN.touchLook.sy = 0;
          IN.touchLook.dx = 0; IN.touchLook.dy = 0; IN.touchLook.idle = 0;
          if (S_ && S_.lookMode === 'stick') {
            sty(lookStickEl, 'left', (t.clientX - 66) + 'px');
            sty(lookStickEl, 'top', (t.clientY - 66) + 'px');
            addC(lookStickEl, 'on');
          }
        }
      }
    ...
    function move(e) {
      ...
      for (var i = 0; i < touches.length; i++) {
        ...
        if (id === IN.stick.id) {
          ...
          IN.touchLook.lx = t.clientX; IN.touchLook.ly = t.clientY;
          IN.touchLook.idle = 0;
          /* absolute deflection, used when lookMode is 'stick' */
          var ldx = t.clientX - IN.touchLook.ox, ldy = t.clientY - IN.touchLook.oy;
          var llen = Math.sqrt(ldx * ldx + ldy * ldy), lmax = 58;
          if (llen > lmax) { ldx = ldx / llen * lmax; ldy = ldy / llen * lmax; }
          IN.touchLook.sx = ldx / lmax; IN.touchLook.sy = ldy / lmax;
          var lnub = lookStickEl && lookStickEl.children && lookStickEl.children[0];
          if (lnub) { sty(lnub, 'transform', 'translate(' + ldx + 'px,' + ldy + 'px)'); }
    ...
    function up(e) {
      ...
      for (var i = 0; i < touches.length; i++) {
        ...
        if (id === IN.stick.id) {
          ...
          remC(lookStickEl, 'on');
          var ln2 = lookStickEl && lookStickEl.children && lookStickEl.children[0];
          if (ln2) { sty(ln2, 'transform', 'translate(0,0)'); }
        }
    ...
    var releaseAll = function () {
      ...
      IN.touchLook.sx = 0; IN.touchLook.sy = 0;
      remC(lookStickEl, 'on');
      var ln3 = lookStickEl && lookStickEl.children && lookStickEl.children[0];
      if (ln3) { sty(ln3, 'transform', 'translate(0,0)'); }
      IN.radial.open = false; IN.radial.sel = -1;
      remC(radialEl, 'on');
      var kids = touchLayer.children || [];
      for (var k = 0; k < kids.length; k++) {
    ...
    Input_.applyTouchStyle = applyTouchStyle;
  ...
  function pollInput(dt) {
    ...
    var aim, fire;
    if (IN.isTouch) {
      /* Synthetic mouse events from taps are ignored outright in touch mode. */
      IN.mouse.b[0] = false; IN.mouse.b[1] = false; IN.mouse.b[2] = false;
      IN.mouseEdge[0] = false; IN.mouse.dx = 0; IN.mouse.dy = 0;
      lookX = 0; lookY = 0;
      aim = false; fire = false;
    } else {
      aim = IN.mouse.b[2];
      fire = IN.mouse.b[0];
    ...
    if (IN.isTouch) {
      ...
      /* If a look pointer is live but has sent no movement for a while, the
         touchend was lost - drop it rather than let a stale finger drift. */
      if (IN.touchLook.id !== -1) {
        IN.touchLook.idle += dt;
        if (IN.touchLook.idle > 0.5) {
          IN.touchLook.id = -1; IN.touchLook.dx = 0; IN.touchLook.dy = 0;
        }
      }
      var ts = (S_ ? S_.touchSensitivity : 1) * 1.35;
      if (S_ && S_.lookMode === 'stick') {
        /* Absolute deflection -> turn rate. A stick cannot drift: it is zero
           whenever the finger is not down. */
        if (IN.touchLook.id !== -1) {
          lookX += IN.touchLook.sx * 950 * dt * ts;
          lookY += IN.touchLook.sy * 700 * dt * ts;
        }
        IN.touchLook.dx = 0; IN.touchLook.dy = 0;
        ts = 0;   /* skip the swipe accumulation below */
      }
      /* Cap a single poll's turn. A duplicated or re-used touch identifier can
         otherwise produce one enormous delta that snaps the view around. */
      var capPx = (hasWin() ? window.innerWidth : 800) * 0.30;
      var tdx = IN.touchLook.dx, tdy = IN.touchLook.dy;
      if (tdx > capPx) { tdx = capPx; } else if (tdx < -capPx) { tdx = -capPx; }
      if (tdy > capPx) { tdy = capPx; } else if (tdy < -capPx) { tdy = -capPx; }
      lookX += tdx * ts;
      lookY += tdy * ts;
      IN.touchLook.dx = 0; IN.touchLook.dy = 0;
  ...
  var Input_ = {
    ...
    state: function () {
      var pads = 0;
      try { pads = pollGamepad() ? 1 : 0; } catch (e) { pads = 0; }
      return {
        touches: (IN.stick.id !== -1 ? 1 : 0) + (IN.touchLook.id !== -1 ? 1 : 0),
        stickId: IN.stick.id, lookId: IN.touchLook.id,
        moveX: OUT.moveX, moveY: OUT.moveY, lookX: OUT.lookX, lookY: OUT.lookY,
        aim: OUT.aim, fire: OUT.fire,
        lookMode: (S_ && S_.lookMode) || 'stick',
        mouseBtn: (IN.mouse.b[0] ? 'L' : '') + (IN.mouse.b[2] ? 'R' : '') || '-',
        pad: pads
      };
    }
  };
  ...
  function initUI(root) {
    ...
    el.dbg = mk('div', 'ip-dbg', el.hud, null);
  ...
  /* Every screen: compact header, ONE scrolling body, a footer that the body
     can never overlap. Built for ~390px of height in landscape. */
  function mkScreen(root, titleText, subText) {
    var scr = mk('div', 'ip-scr', root, null);
    if (subText) { mk('h3', null, scr, subText); }
    var h = mk('h1', null, scr, titleText);
    var body = mk('div', 'ip-body', scr, null);
    var foot = mk('div', 'ip-foot', scr, null);
    return { scr: scr, title: h, body: body, foot: foot };
  }
  ...
  function buildScreens(root) {
    ...
    /* title */
    S1 = mkScreen(root, 'Presidential Extraction', 'Island Protocol');
    el.title = S1.scr;
    el.btnContinue = mbtn(S1.body, 'Continue', function () { cmd('load'); });
    mbtn(S1.body, 'New Game', function () { cmd('newgame'); show('hud'); });
    mbtn(S1.body, 'Settings', function () { show('settings'); });
    el.titleTip = mk('div', 'ip-tip', S1.body, '');
    sty(el.titleTip, 'marginTop', '10px');
    sty(el.titleTip, 'fontSize', '10px');
    sty(el.titleTip, 'opacity', '.5');
    sty(el.titleTip, 'maxWidth', '420px');
    sty(el.titleTip, 'textAlign', 'center');
    sty(el.titleTip, 'lineHeight', '1.7');
    sty(el.titleTip, 'letterSpacing', '.08em');

    /* pause */
    S1 = mkScreen(root, 'Paused', null);
    el.pause = S1.scr;
    mbtn(S1.body, 'Resume', function () { cmd('resume'); });
    mbtn(S1.body, 'Save', function () { cmd('save'); });
    mbtn(S1.body, 'Settings', function () { show('settings'); });
    mbtn(S1.body, 'Restart Section', function () { cmd('restart'); });
    mbtn(S1.body, 'Quit to Title', function () { show('title'); });

    /* settings */
    S1 = mkScreen(root, 'Settings', null);
    el.settings = S1.scr;
    buildSettingsRows(S1.body);
    mbtn(S1.foot, 'Back', function () { show(currentPrev || 'title'); });

    /* inventory */
    S1 = mkScreen(root, 'Attache Case', null);
    el.inv = S1.scr;
    var row = mk('div', 'ip-invrow', S1.body, null);
    el.invGridWrap = mk('div', null, row, null);
    sty(el.invGridWrap, 'position', 'relative');
    el.invGrid = mk('div', 'ip-grid', el.invGridWrap, null);
    var side = mk('div', 'ip-side2', row, null);
    el.invName = mk('div', 'nm', side, 'Attache Case');
    el.invInfo = mk('div', 'ds', side, 'Select an item to inspect it. Drag to move, tap ROTATE to turn it, drop one item onto another to combine.');
    mbtn(S1.foot, 'Rotate', function () {
      if (!lastState || selectedUid < 0) { return; }
      var it = findItem(lastState, selectedUid);
      if (!it) { return; }
      if (IP.Systems.Inventory.move(lastState, it.uid, it.x, it.y, !it.rot)) {
        renderInventory(lastState);
        if (Audio_ && Audio_.play) { Audio_.play('ui_move'); }
      }
    });
    mbtn(S1.foot, 'Sort', function () {
      if (IP.Systems && lastState) { IP.Systems.Inventory.sort(lastState); renderInventory(lastState); }
    });
    mbtn(S1.foot, 'Close', function () { cmd('resume'); });

    /* game over */
    S1 = mkScreen(root, 'Mission Failed', null);
    el.gameover = S1.scr;
    el.goTitle = S1.title;
    el.goText = mk('div', 'ip-quote', S1.body, '');
    sty(el.goText, 'maxWidth', '520px');
    sty(el.goText, 'textAlign', 'center');
    sty(el.goText, 'fontSize', '11px');
    sty(el.goText, 'opacity', '.72');
    sty(el.goText, 'lineHeight', '1.8');
    sty(el.goText, 'marginBottom', '10px');
    mbtn(S1.foot, 'Retry', function () { cmd('restart'); });
    mbtn(S1.foot, 'Load Save', function () { cmd('load'); });
    mbtn(S1.foot, 'Quit to Title', function () { show('title'); });

    /* results */
    S1 = mkScreen(root, 'Extracted', null);
    el.results = S1.scr;
    el.resTitle = S1.title;
    el.resRank = mk('div', 'ip-rank', S1.body, '');
    sty(el.resRank, 'fontSize', 'clamp(30px,9vh,58px)');
    sty(el.resRank, 'letterSpacing', '.1em');
    sty(el.resRank, 'margin', '2px 0 10px');
    el.resText = mk('div', null, S1.body, '');
    sty(el.resText, 'maxWidth', '520px');
    sty(el.resText, 'textAlign', 'center');
    sty(el.resText, 'fontSize', '11px');
    sty(el.resText, 'opacity', '.75');
    sty(el.resText, 'lineHeight', '1.8');
    mbtn(S1.foot, 'New Game +', function () { cmd('newgame'); show('hud'); });
    mbtn(S1.foot, 'Quit to Title', function () { show('title'); });

    /* document reader */
    S1 = mkScreen(root, 'Document', null);
    el.doc = S1.scr;
    el.docTitle = S1.title;
    el.docBody = mk('div', null, S1.body, '');
    sty(el.docBody, 'maxWidth', '620px');
    sty(el.docBody, 'fontSize', '11px');
    sty(el.docBody, 'lineHeight', '1.9');
    sty(el.docBody, 'whiteSpace', 'pre-wrap');
    sty(el.docBody, 'opacity', '.84');
    sty(el.docBody, 'textAlign', 'left');
    mbtn(S1.foot, 'Close', function () { cmd('resume'); });
  }

  function findItem(S, uid) {
    for (var i = 0; i < S.inventory.items.length; i++) {
      if (S.inventory.items[i].uid === uid) { return S.inventory.items[i]; }
    }
    return null;
  }
  ...
  function section(parent, label) { mk('div', 'ip-sect', parent, label); }

  function buildSettingsRows(wrap) {
    var pct = function (v) { return Math.round(v * 100) + '%'; };

    section(wrap, 'Display');
    cycleRow(wrap, 'Graphics', 'quality', [0, 1, 2], ['Low', 'Medium', 'High']);
    sliderRow(wrap, 'Brightness', 'brightness', 0.5, 1.8, 0.05, pct);

    section(wrap, 'Controls');
    cycleRow(wrap, 'Scheme', 'controlLayout', ['auto', 'touch', 'desktop'],
             ['Auto', 'Touch', 'Keyboard']);
    ...
    cycleRow(wrap, 'Look Control', 'lookMode', ['stick', 'swipe'],
             ['Right Stick', 'Swipe']);
    cycleRow(wrap, 'Aim Mode', 'aimToggle', [false, true], ['Hold', 'Toggle']);
    cycleRow(wrap, 'Invert Look', 'invertY', [false, true], ['Off', 'On']);
    cycleRow(wrap, 'Left Handed', 'leftHanded', [false, true], ['Off', 'On']);
    sliderRow(wrap, 'Control Size', 'touchScale', 0.75, 1.5, 0.05, pct);
    sliderRow(wrap, 'Control Opacity', 'touchOpacity', 0.25, 1.0, 0.05, pct);

    section(wrap, 'Audio');
    sliderRow(wrap, 'Master', 'volMaster', 0, 1, 0.05, pct);
    sliderRow(wrap, 'Music', 'volMusic', 0, 1, 0.05, pct);
    sliderRow(wrap, 'Effects', 'volSfx', 0, 1, 0.05, pct);

    section(wrap, 'Difficulty');
    cycleRow(wrap, 'Threat Level', 'difficulty', ['easy', 'normal', 'hard', 'pro'],
             ['Assisted', 'Standard', 'Veteran', 'Professional']);

    section(wrap, 'Accessibility');
    ...
    cycleRow(wrap, 'Input Diagnostic', 'inputDebug', [false, true], ['Off', 'On']);
  }

  function applyTouchStyle() {
    if (!touchLayer) { return; }
    var sc = (S_ && S_.touchScale) || 1;
    var op = (S_ && S_.touchOpacity) === undefined ? 0.85 : S_.touchOpacity;
    var kids = touchLayer.children || [];
    for (var i = 0; i < kids.length; i++) {
      var a = kids[i].getAttribute && kids[i].getAttribute('data-act');
      if (!a) { continue; }
      sty(kids[i], 'transform', 'scale(' + sc + ')');
      sty(kids[i], 'opacity', String(op));
    }
    if (stickEl) { sty(stickEl, 'opacity', String(op)); }
  }

  function applyLiveSettings(key, v) {
    if (key === 'quality') { cmd('quality', v); }
    else if (key === 'touchScale' || key === 'touchOpacity') { applyTouchStyle(); }
  ...
  /* Small procedural icon per item kind - no image files anywhere. */
  function itemIcon(kind, w, h) {
    var c = mk('canvas', null, null, null);
    if (!c || !c.getContext) { return null; }
    c.width = w; c.height = h;
    var g = c.getContext('2d');
    if (!g) { return null; }
    var cx = w / 2, cy = h / 2, u = Math.min(w, h) * 0.30;
    g.lineWidth = Math.max(1.2, u * 0.16);
    g.lineJoin = 'round';
    var col = { weapon: '#c8ced6', ammo: '#c9a86a', heal: '#7fdca0',
                resource: '#9aa6b4', treasure: '#e0c060', key: '#cfd6dc',
                doc: '#b9c0c8', throwable: '#d08a6a' }[kind] || '#aab2bb';
    g.strokeStyle = col; g.fillStyle = col;
    if (kind === 'weapon') {
      g.beginPath(); g.moveTo(cx - u, cy - u * 0.25); g.lineTo(cx + u, cy - u * 0.25);
      g.lineTo(cx + u, cy + u * 0.1); g.lineTo(cx + u * 0.1, cy + u * 0.1);
      g.lineTo(cx - u * 0.35, cy + u); g.lineTo(cx - u, cy + u); g.closePath(); g.stroke();
    } else if (kind === 'ammo') {
      for (var i = -1; i <= 1; i++) {
        g.beginPath();
        g.roundRect ? g.roundRect(cx + i * u * 0.62 - u * 0.22, cy - u * 0.7, u * 0.44, u * 1.4, u * 0.2)
                    : g.rect(cx + i * u * 0.62 - u * 0.22, cy - u * 0.7, u * 0.44, u * 1.4);
        g.stroke();
      }
    } else if (kind === 'heal') {
      g.beginPath(); g.moveTo(cx, cy - u); g.lineTo(cx, cy + u);
      g.moveTo(cx - u, cy); g.lineTo(cx + u, cy); g.stroke();
    } else if (kind === 'treasure') {
      g.beginPath(); g.moveTo(cx, cy - u); g.lineTo(cx + u, cy); g.lineTo(cx, cy + u);
      g.lineTo(cx - u, cy); g.closePath(); g.stroke();
    } else if (kind === 'key') {
      g.beginPath(); g.arc(cx - u * 0.4, cy, u * 0.5, 0, 6.283); g.stroke();
      g.beginPath(); g.moveTo(cx + u * 0.1, cy); g.lineTo(cx + u, cy);
      g.moveTo(cx + u * 0.75, cy); g.lineTo(cx + u * 0.75, cy + u * 0.45); g.stroke();
    } else if (kind === 'doc') {
      g.strokeRect(cx - u * 0.65, cy - u, u * 1.3, u * 2);
      g.beginPath();
      for (var l = -1; l <= 1; l++) {
        g.moveTo(cx - u * 0.4, cy + l * u * 0.5); g.lineTo(cx + u * 0.4, cy + l * u * 0.5);
      }
      g.lineWidth = Math.max(1, u * 0.1); g.stroke();
    } else {
      g.beginPath(); g.arc(cx, cy, u * 0.85, 0, 6.283); g.stroke();
    }
    return c;
  }

  function describeItem(def) {
    if (!def) { return ''; }
    var bits = [];
    if (def.kind === 'heal') { bits.push('Restores ' + Math.round((def.heal || 0) * 100) + '% health.'); }
    else if (def.kind === 'ammo') { bits.push('Ammunition. ' + (def.per || 10) + ' rounds per pickup.'); }
    else if (def.kind === 'weapon') { bits.push('Weapon. Equip from the case.'); }
    else if (def.kind === 'treasure') { bits.push('Valuable. Sell to the quartermaster.'); }
    else if (def.kind === 'key') { bits.push('Opens a specific lock on the island.'); }
    else if (def.kind === 'doc') { bits.push('Recovered document.'); }
    else { bits.push('Crafting resource.'); }
    bits.push('Size ' + def.w + 'x' + def.h + (def.stack > 1 ? '  -  stacks to ' + def.stack : ''));
    if (def.value) { bits.push('Value ' + def.value); }
    return bits.join('\n');
  }

  function selectItem(S, uid) {
    selectedUid = uid;
    var it = findItem(S, uid);
    var def = it && IP.Systems.ITEMS[it.item];
    setText(el.invName, def ? def.name : 'Attache Case');
    setText(el.invInfo, def ? describeItem(def)
      : 'Select an item to inspect it. Drag to move, tap ROTATE to turn it, drop one item onto another to combine.');
    var kids = el.invGridWrap.querySelectorAll('.ip-gitem');
    for (var i = 0; i < kids.length; i++) {
      togC(kids[i], 'sel', String(kids[i].__uid) === String(uid));
    }
  }
  ...
  function renderInventory(S) {
    ...
    var inv = S.inventory;
    var availW = (hasWin() ? window.innerWidth : 900) * 0.52;
    var availH = (hasWin() ? window.innerHeight : 500) - 150;
    var cell = Math.floor(Math.min(availW / inv.w, availH / inv.h, 46));
    if (!(cell > 8)) { cell = 22; }
    ...
    for (i = 0; i < inv.items.length; i++) {
      ...
      var d = mk('div', 'ip-gitem', el.invGridWrap, null);
      d.__uid = it.uid;
      sty(d, 'left', (4 + it.x * step) + 'px');
      sty(d, 'top', (4 + it.y * step) + 'px');
      sty(d, 'width', (w * cell + (w - 1) * 2) + 'px');
      sty(d, 'height', (h * cell + (h - 1) * 2) + 'px');
      sty(d, 'flexDirection', 'column');
      var ic = itemIcon(def.kind, Math.min(30, w * cell * 0.7), Math.min(30, h * cell * 0.7));
      if (ic) { d.appendChild(ic); }
      var lbl = mk('span', null, d, (it.qty > 1 ? 'x' + it.qty : ''));
      sty(lbl, 'fontSize', '8px');
      sty(lbl, 'opacity', '.75');
      if (String(it.uid) === String(selectedUid)) { addC(d, 'sel'); }
      wireItemDrag(S, d, it, step);
    }
    if (selectedUid >= 0 && !findItem(S, selectedUid)) { selectedUid = -1; }
    selectItem(S, selectedUid);
  }

  /* Touch/mouse drag: move within the grid, or drop onto another item to
     combine (herb mixing, ammo crafting). */
  function wireItemDrag(S, d, it, step) {
    ...
    var move = function (e) {
      ...
      sty(d, 'transform', 'translate(' + dx + 'px,' + dy + 'px)');
      if (e.preventDefault && e.cancelable) { e.preventDefault(); }
    };
    var end = function (e) {
      if (!dragging) { return; }
      ...
      if (!moved) { selectItem(S, it.uid); if (Audio_ && Audio_.play) { Audio_.play('ui_move'); } return; }
      var gx = ox + Math.round((t.clientX - sx) / step);
      var gy = oy + Math.round((t.clientY - sy) / step);
      /* drop onto another item -> try to combine */
      var target = null, all = S.inventory.items;
      for (var k = 0; k < all.length; k++) {
        var o = all[k];
        if (o.uid === it.uid) { continue; }
        var od = IP.Systems.ITEMS[o.item];
        if (!od) { continue; }
        var ow = o.rot ? od.h : od.w, oh = o.rot ? od.w : od.h;
        if (gx >= o.x && gx < o.x + ow && gy >= o.y && gy < o.y + oh) { target = o; break; }
      }
      if (target && IP.Systems.Inventory.combine(S, it.uid, target.uid)) {
        if (Audio_ && Audio_.play) { Audio_.play('ui_select'); }
      } else if (IP.Systems.Inventory.move(S, it.uid, gx, gy, it.rot)) {
        if (Audio_ && Audio_.play) { Audio_.play('ui_move'); }
      } else if (Audio_ && Audio_.play) { Audio_.play('ui_error'); }
      renderInventory(S);
    };
    on(d, 'touchstart', start, { passive: false });
    on(d, 'touchmove', move, { passive: false });
    on(d, 'touchend', end, { passive: false });
    on(d, 'touchcancel', end, { passive: false });
    on(d, 'mousedown', function (e) {
      start(e);
      var mm = function (ev) { move(ev); };
      var mu = function (ev) { end(ev);
        if (hasWin()) { window.removeEventListener('mousemove', mm); window.removeEventListener('mouseup', mu); } };
      if (hasWin()) { on(window, 'mousemove', mm); on(window, 'mouseup', mu); }
    });
  }
  ...
  function updateUI(S, dt) {
    ...
    /* Input diagnostic: shows exactly what the input layer believes is live.
       If the camera moves on its own, this says which source is producing it. */
    if (S_ && S_.inputDebug && el.dbg) {
      addC(el.dbg, 'on');
      dbgT += dt;
      if (dbgT > 0.1) {
        dbgT = 0;
        var st = IP.Input.state ? IP.Input.state() : null;
        if (st) {
          setText(el.dbg,
            'touches ' + st.touches + '   stick ' + st.stickId + '   look ' + st.lookId + '\n' +
            'move ' + st.moveX.toFixed(2) + ',' + st.moveY.toFixed(2) +
            '   look ' + st.lookX.toFixed(1) + ',' + st.lookY.toFixed(1) + '\n' +
            'aim ' + (st.aim ? 1 : 0) + ' fire ' + (st.fire ? 1 : 0) +
            '   yaw ' + (IP.Game && IP.Game.cam ? IP.Game.cam.yaw.toFixed(3) : '-') + '\n' +
            'mode ' + st.lookMode + '   mouseBtn ' + st.mouseBtn + '   pad ' + (st.pad ? 1 : 0));
        }
      }
    } else if (el.dbg) { remC(el.dbg, 'on'); }
