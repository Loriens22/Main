...
export function buildRoute() {
  ...
  const S3 = mkStreet('S3', 'бул. „Гоце Делчев“', new Turtle(I2.x, I2.z + 400, NORTH).line(400 + 560 + 400).build(), { wires: true });
  const I3 = { x: I2.x, z: I2.z - 560 };
  const S4 = mkStreet('S4', 'ул. „Пчела“ / „Ген. Тошев“', new Turtle(I3.x - 400, I3.z, EAST).line(400 + 1200).build(), { wires: true, parkingLeft: true });
  ...
  f = fillet(pts, lanePts(S4, 400, 1600, 5.25), 12.5); pts = f.pts;
  ...
  let op = lanePts(S4, 1600, 400, -5.25); // travelling west on S4 → right side is north (negative street offset)
